...
function getPostSignUpDestination(emailAddress: string) {
  return ADMIN_EMAIL_WHITELIST.has(emailAddress.trim().toLowerCase()) ? "/admin" : "/onboarding";
}
...
export default function SignUpPage() {
  ...
  const handleSocialSignUp = async (strategy: OAuthStrategy) => {
    ...
    await signUp.sso({
      ...
      redirectCallbackUrl: `${window.location.origin}${getPostSignUpDestination(email)}`,
    });
  };
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signUp.status === "complete") {
        ...
        router.replace(getPostSignUpDestination(email));
      }
    } catch (err: unknown) {
      const e = err as { errors?: { message: string }[]; message?: string };
